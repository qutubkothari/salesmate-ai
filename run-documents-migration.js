/**
 * Run Document Generation Migration
 * Creates document tables: templates, generated docs, versions, signatures
 */
...
console.log('🚀 Running Document Generation Migration...\n');

try {
  // Read migration SQL
  const migrationPath = path.join(__dirname, 'migrations', 'create_document_generation.sql');
  ...
  const tables = db.prepare(`
    ...
    WHERE type='table' AND (name LIKE 'document_%' OR name = 'document_branding')
    ORDER BY name
  `).all();
  ...
  console.log('\n✅ Document Generation migration completed successfully!');
