...
const sendPDFViaWhatsApp = async (phoneNumber, pdfBuffer, filename, caption = '') => {
    try {
        ...
        if (uploadResult.success) {
            ...
            return { success: true, method: 'gcs_link', fileUrl: uploadResult.url, messageId: null };
        } else {
        ...
        return { success: true, method: 'notification_fallback', fileUrl: null, messageId: null };
    }
};

/**
 * Prefer WhatsApp Web direct document sending when available.
 * Falls back to link-based delivery (Uploadcare/GCS) if WA Web fails.
 */
const sendPDFToCustomer = async (tenantId, phoneNumber, pdfBuffer, filename, caption = '') => {
    // Try WhatsApp Web first (no external hosting required)
    try {
        const { sendWebDocumentMessage } = require('./whatsappWebService');
        if (typeof sendWebDocumentMessage === 'function') {
            const waRes = await sendWebDocumentMessage(tenantId, phoneNumber, caption, pdfBuffer, filename, 'application/pdf');
            return { success: true, method: 'wa_web_document', fileUrl: null, messageId: waRes?.messageId || null };
        }
    } catch (e) {
        console.warn('[PDF_WHATSAPP] WhatsApp Web document send failed, falling back to link method:', e?.message || e);
    }

    // Fallback: link-based upload + message
...
const deliverOrderPDF = async (tenantId, orderId, phoneNumber) => {
    try {
        ...
        const deliveryResult = await sendPDFToCustomer(
            tenantId,
            phoneNumber,
        ...
        // Step 4 (best-effort): If an invoice was created, generate and send invoice PDF too.
        // This keeps the checkout experience "complete" for customers requesting invoices.
        try {
            const zohoInvoiceId = zohoResult?.zohoInvoiceId;
            if (zohoInvoiceId) {
                console.log('[PDF_DELIVERY] Invoice detected; generating invoice PDF:', zohoInvoiceId);
                const { generateInvoicePDF } = require('./zohoInvoiceService');
                const invoicePdf = await generateInvoicePDF(tenantId, zohoInvoiceId);

                if (invoicePdf?.success && invoicePdf.pdfBuffer) {
                    await sendPDFToCustomer(
                        tenantId,
                        phoneNumber,
                        invoicePdf.pdfBuffer,
                        invoicePdf.filename || `invoice_${zohoInvoiceId}_${Date.now()}.pdf`,
                        `📄 Your invoice from ${new Date().toLocaleDateString()}\n\nOrder ID: ${orderId.substring(0, 8)}\nInvoice: ${String(zohoInvoiceId).substring(0, 8)}\n\nThank you for your business!`
                    );
                    console.log('[PDF_DELIVERY] Invoice PDF sent successfully');
                } else {
                    console.warn('[PDF_DELIVERY] Invoice PDF generation failed:', invoicePdf?.error || 'unknown error');
                }
            }
        } catch (invErr) {
            console.warn('[PDF_DELIVERY] Invoice delivery skipped due to error:', invErr?.message || invErr);
        }
