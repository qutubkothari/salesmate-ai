 ...
 * Get product images
 */
const getProductImages = async (productCode, tenantId = null) => {
  try {
    // STEP 1: Check if product has image_url in database
    if (tenantId) {
      try {
        ...
        if (products && products.length > 0) {
          ...
          if (productsWithImages.length > 0) {
            console.log(`[PRODUCT_IMAGES] Found ${productsWithImages.length} products with image URLs in database`);
            return {
              success: true,
              images: productsWithImages.map(p => ({
                name: p.name,
                signedUrl: p.image_url,
                source: 'database'
              }))
            };
          }
        ...
        console.log('[PRODUCT_IMAGES] Database check failed, falling back to GCS:', dbError.message);
      }
    }

    // STEP 2: Fall back to GCS uploaded files
...
const handleDocumentRequest = async (requestType, productCode, tenantId) => {
  try {
    ...
    let result;
    let responseMessage = '';
    let imageData = null; // For sending images directly
    ...
    switch (requestType) {
      ...
      case 'PRODUCT_IMAGE':
        ...
        result = await getProductImages(productCode, tenantId);
        if (result.success && result.images.length > 0) {
          // Return image data for direct sending instead of URLs
    ...
    return { success: true, message: responseMessage, images: imageData };
