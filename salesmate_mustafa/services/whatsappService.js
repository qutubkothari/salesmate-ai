...
const sendMessageWithImage = async (to, caption, mediaUrl) => {
    // Check if we're in desktop agent mode - if so, capture instead of sending
    if (global.desktopAgentMode) {
        console.log('[WHATSAPP_SERVICE] Desktop agent mode - capturing image message instead of sending');
        if (!Array.isArray(global.capturedMessages)) {
            global.capturedMessages = [];
        }
        global.capturedMessages.push({
            type: 'image',
            url: mediaUrl,
            caption: caption
        });
        return 'desktop_agent_captured_image_' + Date.now();
    }
    ...
    try {
        ...
        return responseBody.data?.message_id || responseBody.message_id || responseBody.data?.id || null;
    } catch (error) {
