...
function calculatePricingBreakdown(priceBeforeTax, quantity) {
    const unitPriceBeforeTax = Number(priceBeforeTax) || 0;
    const gstRate = 18;
    const gstAmount = unitPriceBeforeTax * 0.18 * (Number(quantity) || 0);

    // Store per-unit price (before tax) to align with dashboard calculations.
    return {
        price_at_time_of_purchase: unitPriceBeforeTax,
...
class EnhancedOrderProcessingWithZoho {
    ...
    async createLocalOrder(tenantId, phoneNumber, orderDetails) {
        try {
            ...
            const phoneForOrdersTable = normalizedPhone.endsWith('@c.us')
                ? normalizedPhone.replace('@c.us', '')
                : normalizedPhone;

            // Get conversation
            let conversationRes = await supabase
                .from('conversations')
                .select('id')
                .eq('tenant_id', tenantId)
                .eq('end_user_phone', normalizedPhone)
                .maybeSingle();

            if (!conversationRes?.data && phoneForOrdersTable) {
                conversationRes = await supabase
                    .from('conversations')
                    .select('id')
                    .eq('tenant_id', tenantId)
                    .eq('end_user_phone', phoneForOrdersTable)
                    .maybeSingle();
            }

            // Older SQLite schema often stores numeric phone in `phone_number`
            if (!conversationRes?.data && phoneForOrdersTable) {
                conversationRes = await supabase
                    .from('conversations')
                    .select('id')
                    .eq('tenant_id', tenantId)
                    .eq('phone_number', phoneForOrdersTable)
                    .maybeSingle();
            }
            ...
            const { data: order, error: orderError } = await supabase
                ...
                .insert({
                    ...
                    // Some SQLite schemas have NOT NULL phone_number
                    phone_number: phoneForOrdersTable,
            ...
            if (orderDetails.items && orderDetails.items.length > 0) {
                const orderItems = orderDetails.items.map(item => {
                    const unitPriceBeforeTax = Number(item.price) || 0;
                ...
                // Some older schemas may not have tenant_id on order_items.
                if (itemsInsert?.error && /no such column: tenant_id|has no column named tenant_id|column\s+"tenant_id"\s+does not exist/i.test(String(itemsInsert.error.message || itemsInsert.error))) {
                    const retryItems = orderItems.map(({ tenant_id, ...rest }) => rest);
            ...
            if (zohoResult.success) {
                ...
                try {
                    ...
                    if (!zohoResult.pdfBuffer && zohoResult.zohoOrderId) {
                        ...
                        const pdfResult = await getZohoSalesOrderPDF(tenantId, zohoResult.zohoOrderId);
