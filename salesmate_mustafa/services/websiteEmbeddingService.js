...
async function generateEmbedding(text, model = null) {
    // Prefer free/local embeddings where possible, but never let missing HF tokens break semantic search.
    const envWantsOpenAI = String(process.env.USE_OPENAI_EMBEDDINGS || '').toLowerCase() === 'true';
    const hasHfToken = !!(process.env.HUGGINGFACE_API_KEY || '').trim();
    ...
    const shouldTryFreeFirst = !envWantsOpenAI && (canUseLocal || hasHfToken);

    if (shouldTryFreeFirst) {
        try {
            ...
            const hfMissing = /HUGGINGFACE_API_KEY/i.test(msg);
            if (!hasOpenAiKey) {
                console.error('[EmbeddingService] Error generating embedding (free/local):', msg);
                throw error;
            }
            if (hfMissing) {
                console.warn('[EmbeddingService] HF token missing; falling back to OpenAI embeddings');
            } else {
                console.warn('[EmbeddingService] Free/local embedding failed; falling back to OpenAI:', msg);
            }
        }
    }

    if (!hasOpenAiKey) {
        // No OpenAI key; last resort: try free/local anyway (will throw a useful error).
        const embedding = await generateSmartEmbedding(text);
        return embedding;
    }
    ...
    try {
        ...
        console.error('[EmbeddingService] Error generating OpenAI embedding:', error.message);
        throw error;
    }
}
...
async function generateEmbeddingsBatch(texts, model = null) {
    ...
    // If HF token is missing and local model isn't installed, free batch will mostly produce zero vectors.
    const shouldUseOpenAiBatch = envWantsOpenAI || (!canUseLocal && !hasHfToken && hasOpenAiKey);

    try {
        if (!shouldUseOpenAiBatch) {
