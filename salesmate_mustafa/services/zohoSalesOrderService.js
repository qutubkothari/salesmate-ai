...
/**
 * Legacy (env-based) token refresh
 */
const getLegacyZohoAccessToken = async () => {
    try {
        if (!ZOHO_CONFIG.refreshToken || !ZOHO_CONFIG.clientId || !ZOHO_CONFIG.clientSecret) {
            throw new Error('Legacy Zoho credentials not configured (missing refresh token/client id/secret)');
        }
...
/**
 * Get Zoho auth context for a tenant.
 * - Prefers legacy env-based config (single-tenant mode) if fully configured.
 * - Otherwise uses tenant-based Zoho auth stored on the tenant record.
 */
const getZohoAuthContext = async (tenantId) => {
    const hasLegacy = !!(ZOHO_CONFIG.refreshToken && ZOHO_CONFIG.clientId && ZOHO_CONFIG.clientSecret && ZOHO_CONFIG.organizationId);
    if (hasLegacy) {
        const accessToken = await getLegacyZohoAccessToken();
        return {
            accessToken,
            organizationId: ZOHO_CONFIG.organizationId,
            apiUrl: ZOHO_CONFIG.apiUrl
        };
    }

    if (!tenantId) {
        throw new Error('tenantId is required for tenant-based Zoho auth');
    }

    const { accessToken, organizationId } = await zohoTenantAuth.getValidAccessToken(tenantId);
    if (!organizationId) {
        throw new Error('Zoho organization ID is missing for this tenant');
    }
...
/**
 * Backwards-compatible token getter.
 */
const getZohoAccessToken = async (tenantId) => {
    const { accessToken } = await getZohoAuthContext(tenantId);
    return accessToken;
};
...
const findOrCreateZohoCustomer = async (tenantId, endUserPhone) => {
    try {
        ...
        const { accessToken, organizationId, apiUrl } = await getZohoAuthContext(tenantId);
        ...
        if (customerProfile.company) {
            ...
            const companySearchResponse = await fetch(
                `${apiUrl}/contacts?company_name=${encodeURIComponent(customerProfile.company)}&organization_id=${organizationId}`,
                {
                    headers: {
        ...
        const searchResponse = await fetch(
            `${apiUrl}/contacts?phone=${encodeURIComponent(customerProfile.phone)}&organization_id=${organizationId}`,
            {
                headers: {
                    'Authorization': `Zoho-oauthtoken ${accessToken}`,
                    'Content-Type': 'application/json'
            ...
            if (searchData.contacts && searchData.contacts.length > 0) {
                ...
                if (shouldUpdate && (customerProfile.first_name || customerProfile.company)) {
                    ...
                    const updateResponse = await fetch(
                        `${apiUrl}/contacts/${zohoCustomer.contact_id}?organization_id=${organizationId}`,
                        {
                            method: 'PUT',
                            headers: {
                                'Authorization': `Zoho-oauthtoken ${accessToken}`,
        ...
        if (!zohoCustomer) {
            ...
            const createResponse = await fetch(`${apiUrl}/contacts?organization_id=${organizationId}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Zoho-oauthtoken ${accessToken}`,
                    'Content-Type': 'application/json'
...
const createZohoSalesOrder = async (tenantId, orderId) => {
    try {
        ...
        const response = await fetch(`${apiUrl}/salesorders?organization_id=${organizationId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Zoho-oauthtoken ${accessToken}`,
                'Content-Type': 'application/json'
        ...
        if (responseData.salesorder) {
            ...
            const pdfResult = await getZohoSalesOrderPDF(tenantId, zohoOrderId);
...
const processOrderToZoho = async (tenantId, orderId) => {
    try {
        ...
        const pdfResult = await getZohoSalesOrderPDF(tenantId, orderResult.zohoOrderId);
        ...
        return {
            success: true,
            zohoOrderId: orderResult.zohoOrderId,
            zohoInvoiceId: orderResult.zohoInvoiceId,
 ...
 */
const updateZohoOrderStatus = async (tenantId, zohoOrderId, status) => {
    try {
        const { accessToken, organizationId, apiUrl } = await getZohoAuthContext(tenantId);
        ...
        const response = await fetch(`${apiUrl}/salesorders/${zohoOrderId}?organization_id=${organizationId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Zoho-oauthtoken ${accessToken}`,
                'Content-Type': 'application/json'
 ...
 */
const getZohoSalesOrderPDF = async (tenantId, zohoOrderId) => {
    try {
        const { accessToken, organizationId, apiUrl } = await getZohoAuthContext(tenantId);
        const url = `${apiUrl}/salesorders/${zohoOrderId}?accept=pdf&organization_id=${organizationId}`;
...
const updateSalesOrderNotes = async (tenantId, zohoSalesOrderId, notes) => {
    try {
        ...
        const getResponse = await fetch(
            `${apiUrl}/salesorders/${zohoSalesOrderId}?organization_id=${organizationId}`,
            {
                method: 'GET',
                headers: {
                    'Authorization': `Zoho-oauthtoken ${accessToken}`,
...
const updateInvoiceNotes = async (tenantId, zohoInvoiceId, notes) => {
    try {
        ...
        // Get current invoice
        const getResponse = await fetch(
            `${apiUrl}/invoices/${zohoInvoiceId}?organization_id=${organizationId}`,
            {
                method: 'GET',
                headers: {
                    'Authorization': `Zoho-oauthtoken ${accessToken}`,
