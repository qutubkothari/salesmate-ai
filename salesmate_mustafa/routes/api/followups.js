...
const { upsertKnowledgeItem } = require('../../services/tenantKnowledgeService');
    ...
    try {
        ...
        const followupsWithCustomers = await Promise.all(
            (followups || []).map(async (followup) => {
                let parsedContext = followup.conversation_context;
...
/**
 * POST /api/followups/:tenantId/:followupId/confirm-knowledge
 * Convert an ATTENTION follow-up into a per-tenant knowledge base entry.
 * Body: { confirmed_answer: string, sources?: array }
 */
router.post('/:tenantId/:followupId/confirm-knowledge', async (req, res) => {
    try {
        const { tenantId, followupId } = req.params;
        const confirmedAnswer = String(req.body?.confirmed_answer || '').trim();
        const sources = req.body?.sources;

        if (!tenantId || !followupId) {
            return res.status(400).json({ success: false, error: 'Missing tenantId or followupId' });
        }
        if (!confirmedAnswer) {
            return res.status(400).json({ success: false, error: 'confirmed_answer is required' });
        }
        ...
        const row = Array.isArray(followUp) ? followUp[0] : followUp;
        if (!row) {
            return res.status(404).json({ success: false, error: 'Follow-up not found' });
        }

        const question = String(row.original_request || row.description || '').trim();
        if (!question) {
            return res.status(400).json({ success: false, error: 'Follow-up has no original_request to save as question' });
        }

        const kbItem = await upsertKnowledgeItem({
            tenantId,
            question,
            answer: confirmedAnswer,
            sources: Array.isArray(sources) ? sources : (row?.conversation_context?.sources || []),
            createdBy: 'dashboard_confirm',
            createdFromFollowupId: followupId
        });

        // Mark follow-up completed so it no longer shows as attention.
        ...
        return res.json({
            success: true,
            knowledge: kbItem,
            message: 'Saved to tenant knowledge and marked follow-up completed'
        });
    } catch (error) {
        console.error('[API] Error confirming knowledge:', error);
        return res.status(500).json({ success: false, error: error.message || String(error) });
    }
});
