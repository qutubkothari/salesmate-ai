...
const WAHA_TIMEOUT_MS = Number(process.env.WAHA_TIMEOUT_MS) || 15000;
...
async function wahaRequest(method, url, data = null, responseType = 'json') {
  ...
  try {
    ...
    if (isTimeout) {
      err.message = `WAHA request timed out after ${WAHA_TIMEOUT_MS}ms: ${method} ${url}`;
    }
    throw err;
  }
}
...
const server = app.listen(PORT, async () => {
    ...
    try {
        ...
        const startupInitTimeoutMs = Number(process.env.WA_WEB_STARTUP_INIT_TIMEOUT_MS) || 20000;

        // Don't let startup hang indefinitely due to DB/network/browser issues.
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`WhatsApp Web startup init timed out after ${startupInitTimeoutMs}ms`)), startupInitTimeoutMs)
        );

        Promise.race([autoInitializeConnectedClients(), timeoutPromise])
          .then(() => console.log('[STARTUP] WhatsApp Web auto-initialization complete'))
          .catch((e) => console.warn('[STARTUP] WhatsApp Web auto-initialization skipped:', e?.message || e));
    } catch (error) {
