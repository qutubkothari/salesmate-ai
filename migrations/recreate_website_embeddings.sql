-- Recreate website_embeddings tables with correct dimensions (384 for FREE embeddings)
-- Run this in Supabase SQL Editor to fix the dimension mismatch

-- Step 1: Drop existing tables (will cascade to dependent objects)
...
DROP FUNCTION IF EXISTS search_website_content;

-- Step 3: Recreate tables with correct vector dimensions (384)
...
CREATE INDEX idx_website_embeddings_tenant 
    ON website_embeddings(tenant_id);

CREATE INDEX idx_website_embeddings_url 
    ON website_embeddings(tenant_id, url);

CREATE INDEX idx_website_embeddings_content_type 
    ON website_embeddings(tenant_id, content_type);

CREATE INDEX idx_website_embeddings_status 
    ON website_embeddings(tenant_id, status);

-- Create vector similarity search index (cosine distance)
CREATE INDEX idx_website_embeddings_vector 
...
CREATE INDEX idx_crawl_jobs_tenant 
...
-- Add comments
COMMENT ON TABLE website_embeddings IS 'Stores crawled website content as text chunks with 384-dimension vector embeddings (FREE Hugging Face model)';
COMMENT ON TABLE crawl_jobs IS 'Tracks website crawling jobs and their status';
COMMENT ON FUNCTION search_website_content IS 'Performs semantic search on website content using 384-dimension vector similarity';

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Tables recreated successfully with 384-dimension vectors!';
    RAISE NOTICE 'You can now crawl websites from the dashboard.';
END $$;
